 ...
 * 必ずこのファイル・PTTRoomPermissions.swift(iOS)・PTTRoomPermissions.kt
 * (Android)の3箇所すべてを同時に更新すること。
 *
 * ここで扱わないrole分岐:
 * - Room作成ボタンの表示可否は role ではなく isAnonymous(Firebase Auth)で
 *   判定する。未入室(=Roomメンバーではない)画面では role という概念自体が
 *   存在しないため、この定数の対象外(RoomSelectView.vue参照)。
 */
export const ROOM_MANAGE_ROLES = ['owner', 'moderator'] as const

export type RoomManageRole = (typeof ROOM_MANAGE_ROLES)[number]

/** BAN実行・録音の開始/停止など、owner/moderatorのみに許可されたUIの表示可否判定に使う。 */
export function canManageRoom(role: string | null | undefined): boolean {
  return role != null && (ROOM_MANAGE_ROLES as readonly string[]).includes(role)
}
