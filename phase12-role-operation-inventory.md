...
**（新たに見つかった論点9、2026-08-05・五十三訂で追記）**
本ドキュメントが扱う「role×操作」（`lib/permissions.js`の`ROOM_OPERATIONS`）
とは別に、2026-08-05時点で「Room開始/終了時刻(schedule)による時間軸の
状態(`before_start`/`in_session`/`after_end`)」という、意図的に独立した
もう一つの軸が`lib/roomSchedule.js`として実装されていることを確認した
（`token.js`・`talk.js`・`messages.js`に`requireInSession`/
`requireNotBeforeStart`ミドルウェアとして組み込み済み）。この軸はrole側の
対応表とAND条件で効く（例:`chat:send`はrole不問だが、`in_session`以外では
時間軸側で一律拒否される）。本ドキュメントの対応表一元化の議論（論点5〜7）
を今後進める際、この時間軸ゲートは「role×操作」の対応表そのものには
含めず、別モジュールとして独立させたままにする設計判断を維持するのが
妥当と考えられる（役割と時間という異なる性質の軸を1つの表に混在させると、
かえって見通しが悪くなるため）。詳細は`SECURITY.md`の該当節・
`brushup-plan.md`五十三訂を参照。
