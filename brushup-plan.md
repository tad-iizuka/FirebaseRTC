...
二十三訂: 2026-07-27（「7.3 未確定の要件」のうち対応ファイル種別・
ファイルサイズ上限・保存先の3項目についてユーザーへ確認した。
(1) 対応ファイル種別は画像・動画・PDFの3種類に確定（README.mdの
Communication Modelには「Video」という独立Event種別は無いため、動画は
`File` Eventの一種として扱う整理とした）。(2) ファイルサイズは画像が
圧縮して1MB以下、動画・PDFは無圧縮のまま送信する方針との回答を得たが、
動画・PDFに実装上の安全弁（上限）を設けるかどうかは別途フォローが必要な
論点として残した。(3) 保存先は「決めかねるので提案してほしい」との回答
だったため、チャット添付専用の新規GCSバケットを新設する案を「7.5 保存先
の提案」として新設し、録音Egressバケットとの分離を推奨する理由（権限
モデルの違い・ライフサイクル要件の違い・オブジェクトの性質の違い・
障害影響範囲の分離）を記載した。この提案自体はユーザーの最終確認待ちで
あり、まだ確定事項ではない）
二十四訂: 2026-07-27（二十三訂で保存先について「専用GCSバケット新設」を
提案したところ、「新規GCSバケット or Firebase Storage、どちらがよいか」
との質問を受けた。両者は実体としては同じGCSバケットであり、Firebase
プロジェクトへの登録有無・コンソールでの可視性が違うだけである旨を整理
した上で、Firebaseコンソールでの一元管理のしやすさ等を理由に**Firebase
Storageバケットを新設する**案を確定案として「7.5」に反映した。ただし
運用方針としてはクライアント直接アクセスは全面拒否し、token-server経由の
署名URL方式に統一する（Firebase Storage SDKによるクライアント直接
アップロード/ダウンロードは採用しない）ことも合わせて明記した。また、
動画・PDFのサイズ上限について「1件あたり100MB」との回答を得たため、
7.3で懸念点として残していた「無制限は危険」という論点を解消し確定
扱いとした）
二十五訂: 2026-07-27（「7.3」で残っていた最後の6項目についてユーザーへ
確認し、すべて確定した。(1) 画像圧縮の実施場所はクライアント側とし、
加えてサーバー側でサムネイル（アイコン用）を生成する仕様を新たに追加した。
(2) 保持期間・削除ポリシーは団体（組織階層）単位で設定可能とし、
デフォルト30日とした。これはPhase11で既に実装済みの「Roomの組織階層への
手動割り当て」を流用できるため、Phase13バッジシステムで問題になった
「ユーザー×団体の所属関係が未実装」という制約には抵触しない旨を7.3に
注記した。(3) プレビュー方式はサムネイル表示に確定。(4) コンテンツ
スキャンは、Room内限定共有で拡散性が低いとの理由により不要と確定。
(5) Guestロールは送受信とも可と確定。(6) 3クライアントの対応順序は
Web→iOS→Androidの順に確定。これにより「7.3 未確定の要件」に挙げていた
全項目が確定し、Phase16の残作業は次アクションitem4記載の実コード確認
のみとなった）
二十六訂: 2026-07-27（アップロードされたリポジトリ一式（HEAD=`7f05240`）を
取得し、次アクションitem4の実コード確認を実施した。
**（1）Text Event実装は本ドキュメントの前提通りであることを確認**:
`token-server/routes/messages.js`（Admin SDK経由の書き込み一本化）・
`firestore.rules`の`messages`サブコレクション（`status=='active'`の
アクティブメンバーのみ読み取り可、書き込みは`allow write: if false`）・
Web(`stores/chat.ts`)/iOS(`PTTChatStore.swift`)/Android
(`PTTChatStore.kt`)の3クライアント（いずれもFirestoreの
`addSnapshotListener`/`onSnapshot`で購読、送信はtoken-server経由の
みでFirestoreへの直接書き込みは行わない）を確認し、いずれも「1.」
「2-F」「7.2」に記載していた設計の通りであった。
**（2）（重要な訂正）保存先について二十四訂の前提が誤っていたことが
判明**: `token-server/routes/recording.js`を確認したところ、既存の
録音Egressは実際には**Firebase Admin SDKのStorage機能(`firebase-admin`の
`storage()`)を一切使っておらず、`@google-cloud/storage`パッケージで
専用のGCSバケット（環境変数`RECORDING_GCS_BUCKET`）・専用のサービス
アカウント認証情報（`RECORDING_GCS_CREDENTIALS_JSON`/
`RECORDING_GCS_KEY_FILE`、Firebase Admin SDKの認証情報とは意図的に
分離）を直接扱う設計**だった（`lib/firebaseAdmin.js`の
`admin.initializeApp()`にも`storageBucket`オプション自体が
渡されておらず、リポジトリ全体を検索しても`firebase-admin`の
`storage()`呼び出しは1箇所も存在しなかった）。つまり、二十四訂で
「Firebase Storageバケットを新設する（Firebaseコンソールでの一元管理の
しやすさを理由に）」とした判断は、実コードを確認せず一般論として
妥当と思われる案を提示したものであり、**このプロジェクトの既存の
設計方針（録音Egressのために意図的にFirebase Admin SDKの標準Storage
機能を使わず、専用バケット・専用サービスアカウントで完結させている）
とは整合しない**。この点をユーザーへ再確認する（詳細は7.5参照）。
なお、Firestoreの`organizations/{orgId}`・`rooms.orgId`フィールドは
想定通り実装済みであることも確認し、保持期間の団体単位設定という
7.3の設計方針自体は実コードとの整合性に問題がないことを確認した）
二十七訂: 2026-07-27（二十六訂で見つかった保存先の訂正について、「録音
Egressも今回まとめてFirebase Storageへ移行するか」をユーザーへ確認した。
検討過程で「バックアップを見据えた場合、GCS(素)とFirebase Storageで
効率差があるか」という質問を受けたため調査し、両者は実体として同じ
Google Cloud Storageであり、オブジェクトバージョニング・Storage Transfer
Service・デュアルリージョン/マルチリージョンのバケットロケーションと
いったバックアップ手段はどちらの方式でも同一に使える（バックアップ効率は
「バケットの種類」ではなく「ロケーション設定・バージョニング等の設定」に
依存する）ことを回答した。これはEgressをどちらの方式にするかの判断には
影響しないことを確認した上で、最終的に**「今回はEgressには一切手を
入れず、チャット添付ファイルのみ新規の専用GCSバケット（録音Egressと同じ
「専用バケット＋専用サービスアカウント」方式、Firebase Storage機能は
使わない）で実装する」ことで確定した**。二十六訂で保留としていた
「保存先の訂正提案」はこれによりユーザー確認済みとなり、7.3・7.5の
「ユーザーの最終確認待ち」という留保を解消する。なお、Egressの秘密鍵
運用見直し(IAM SignBlob化等)は今回バックログ化もせず、対応不要として
扱う)
...
**（2026-07-27追記、二十二訂〜二十五訂）** 上記のうちImage/Fileの2Event
について、チャットへの画像・動画・PDF添付を先行実装したいとの要望を受け、
要件たたき台を「7. Image/File Event（チャット添付ファイル）要件たたき台」
として新設した。対応ファイル種別・サイズ上限・保存先・保持期間・
プレビュー方式・コンテンツスキャン要否・Guestの送信可否・3クライアント
対応順序は二十五訂までにすべて確定したが、実コード確認は未了のため、
ロードマップ上は「Phase 16」として優先順位（既存Phase9〜15への割り込み
順）未確定のまま追加している。
...
### Phase 16: Communication ModelへのImage/File Event追加（チャット添付ファイル）→ **要件確定・実コード確認完了、優先順位のみ未確定（2026-07-27、二十七訂で更新）**
README.mdのCommunication Modelが定義する残りのEvent種別のうち、チャットへの
画像・動画・PDF添付を先行実装したいとの要望を受けて追加した。詳細要件は
「7. Image/File Event 要件たたき台」参照。

**現時点の位置づけ**:
- 対応ファイル種別・サイズ上限・保存先（専用GCSバケット＋専用サービス
  アカウント方式）・画像圧縮の実施場所・保持期間/削除ポリシー・
  プレビュー方式・コンテンツスキャン要否・Guestの送信可否・3クライアント
  の対応順序は、いずれもユーザー回答により**確定済み**（詳細は「7.3」参照）
- 実装着手前に必要な**実コード確認は完了**（二十六訂）。既存のText Event
  実装（`token-server/routes/messages.js`＋`ChatPanel.vue`/
  `PTTChatStore.swift`/`PTTChatStore.kt`）が本ドキュメントの前提通りで
  あることを確認し、あわせて保存先の設計を録音Egressの実装
  （`recording.js`）に合わせて訂正した。録音Egress自体は今回一切変更
  しないことも確定した（二十七訂）
- **優先順位・着手条件（ロードマップ上どこに割り込むか）のみ、依然として
  未確定**。Phase9(安定性)〜Phase15(業界ラベリング)の既存順序に対し
  どこに割り込むかは別途ユーザーと合意する必要がある
...
4. **（2026-07-27 二十二訂で追加、二十五訂で要件確定完了、二十六訂・
   二十七訂で実コード確認完了）Image/File Event（画像・動画・PDF添付）の
   ロードマップ上の優先順位決定**：Phase16として追加したチャット添付機能に
   ついて、要件（対応ファイル種別・サイズ上限・画像圧縮の実施場所・保存先・
   保持期間/削除ポリシー・プレビュー方式・コンテンツスキャン要否・Guest
   ロールの送信可否・3クライアントの対応順序）はすべてユーザー回答で確定し、
   実コード確認（`messages.js`・3クライアントのChat実装・`recording.js`の
   保存先パターン・`organizations.js`のorgId構造）も完了した（詳細は
   「7.3」「7.5」参照）。残る作業は以下の1点のみ。
   - **ロードマップ上の優先順位決定**: Phase16は「要件確定・実コード確認
     完了」の位置づけのままPhase9〜15の既存順序に対しどこに割り込むかが
     未確定。警備業（Phase1）の現場写真送信ニーズと、Phase9(安定性)・
     Phase11(組織階層)等の既存優先課題との兼ね合いをユーザーと合意する
   - （実装の細部として）保持期間の団体単位設定（7.3）を実現する具体的な
     データ構造（`organizations/{orgId}`への追加フィールドか別コレクション
     か）は、実装着手時にPhase11の組織階層実装と合わせて設計する
...
`File`という別々のEvent種別として定義されている。2026-07-27のユーザー回答
により、対応対象は**画像・動画・PDFの3種類**に確定した（二十三訂）。

**（要注意）** README.mdのCommunication Model定義（Voice/Text/Image/File/
Location/Reaction/System/AI Message）には「Video」という独立したEvent種別が
存在しない。本計画では、画像は`Image` Event、動画・PDFはいずれも
`File` Event（汎用ファイル）の一種として扱う案を採用する（`Image`と`File`の
2Event、3ファイル種別という整理）。`Location`/`Reaction`/`System`/
`AI Message`は引き続きスコープ外のまま。
...
| BAN後の既存投稿の扱い | 現状はテキストも読めなくなるのみで、投稿自体の削除は行わない | 画像・動画・PDFはテキストより「残り続けることの重さ」が異なりうるため、削除要否は別途確認が必要 |

### 7.3 要件の確定状況（2026-07-27、二十五訂で全項目確定 / 二十六訂で保存先を訂正）

以下、当初挙げた未確定項目はすべてユーザー回答により確定した
（保存先のみ、実コード確認の結果に基づき二十六訂で内容を訂正している）。

- **対応ファイル種別 → 確定**: 画像・動画・PDFの3種類（7.1参照）。
  README.mdの`File` Eventが本来想定する任意拡張子までの汎用対応は
  今回のスコープ外
- **ファイルサイズ上限 → 確定（2026-07-27、二十四訂）**: 画像は圧縮して
  1MB以下、動画・PDFは無圧縮のまま送信するが、システムとしての安全弁
  として**1件あたり100MB**の上限を設ける（7.3旧版で懸念点として挙げた
  「無制限は危険」という点についてユーザーへ確認し、100MBという具体的な
  上限で確定した）
- **画像圧縮の実施場所 → 確定（2026-07-27、二十五訂）**: 画像はクライアント
  側で圧縮（1MB以下）。加えて、サーバー側（token-server）でサムネイル
  （アイコン用の小サイズ画像）を生成する仕様を追加で確定した
- **保存先 → 確定（2026-07-27、二十四訂で一度確定 → 二十六訂で訂正提案 →
  二十七訂で最終確定）**: 当初「Firebase Storageバケットを新設する」で
  確定していたが、実コード確認の結果、既存の録音Egressが実際には
  Firebase Admin SDKのStorage機能を使わず、専用GCSバケット＋専用
  サービスアカウントで完結させる設計だったことが判明したため、**同じ
  「専用GCSバケット＋専用サービスアカウント」方式（Firebase Storage
  機能は使わない）に確定**した。あわせて「録音Egress自体も今回まとめて
  Firebase Storageへ移行するか」を確認したが、**Egressには一切手を
  入れない**ことで確定した（詳細・技術的な理由は「7.5 保存先の提案」参照）
- **保持期間・削除ポリシー → 確定（2026-07-27、二十五訂）**: 保持期間は
  **団体（組織階層）単位で設定可能**とし、デフォルトは**30日**とする。
  - **（設計上の注記）** これはPhase13バッジシステムで「団体単位の
    マスタ管理はPhase13単体では成立しない」と判断した論拠（十九訂：
    「ユーザー×団体の所属関係」が未実装のため）とは前提が異なる。
    保持期間は「Roomがどの団体に属するか」で決まればよく、これは
    Phase11で既に実装済みの「Roomの組織階層への手動割り当て
    （admin-dashboardのorg-assignment）」がそのまま使える。バッジの
    ケースが必要としていたのは「ユーザーがどの団体に属するか」で、
    これは今回不要（保持期間はRoom単位の設定で足りるため）。無所属Room
    （Phase11で正式な状態として許容済み）にはデフォルト30日を適用する
  - **（二十六訂で実コード確認済み）** `token-server/routes/
    organizations.js`・`rooms.orgId`フィールドの実在を確認し、上記の
    設計判断が実コードと矛盾しないことを検証済み
  - 具体的な設定の保存場所（`organizations/{orgId}`への追加フィールドか、
    別コレクションか）は実装時の設計課題として残す
- **プレビュー方式 → 確定（2026-07-27、二十五訂）**: サムネイル表示とする
  （上記の通りサーバー側で生成）。動画のサムネイル抽出方法（先頭フレーム
  切り出し等）・PDFのサムネイル生成方法（1ページ目の画像化等）の具体的な
  実装方式は実装時に検討する
- **コンテンツスキャン → 確定（2026-07-27、二十五訂）**: 不要とする
  （Room内メンバーへの限定共有であり、広く拡散される性質のものではない
  との判断）
- **Guestロールの送信可否 → 確定（2026-07-27、二十五訂）**: 送受信とも
  可とする（テキストチャットと同様の扱い）
- **3クライアントの対応順序 → 確定（2026-07-27、二十五訂）**: Web →
  iOS → Android の順で段階的に実装する

### 7.5 保存先の提案（2026-07-27、二十三訂で新設 / 二十四訂で確定 / 二十六訂で訂正・再提案 / 二十七訂で最終確定）

「決めかねるので提案してほしい」との回答を受けて提案し（二十三訂）、
続けて「新規GCSバケット or Firebase Storage、どちらがよいか」という質問を
受けたため「Firebase Storageバケットを新設する」で確定していた（二十四訂）。
しかし、その判断は実コードを確認せずに行った一般論であり、二十六訂の
実コード確認によって**このプロジェクトの既存の設計方針とは整合しない**
ことが判明したため、ここで訂正・再提案する。

**判明した事実（二十六訂）**: 既存の録音Egress（`token-server/routes/
recording.js`）は、Firebase Admin SDKのStorage機能（`firebase-admin`の
`storage()`）を一切使っていない。代わりに、`@google-cloud/storage`
パッケージを直接使い、専用のGCSバケット（環境変数`RECORDING_GCS_BUCKET`）・
専用のサービスアカウント認証情報（`RECORDING_GCS_CREDENTIALS_JSON`/
`RECORDING_GCS_KEY_FILE`）を、Firebase Admin SDKの認証情報とは意図的に
分離して扱っている。`lib/firebaseAdmin.js`の`admin.initializeApp()`にも
`storageBucket`オプション自体が渡されておらず、リポジトリ全体を検索しても
`firebase-admin`の`storage()`呼び出しは1箇所も存在しない。

つまり、二十四訂の整理（「Firebase Storageと素のGCSバケットは実体として
同じもの」）自体は技術的に誤りではないが、**「Firebaseコンソールでの
一元管理のしやすさ」を理由にFirebase Storageを推した判断は、このプロジェクトが
録音Egressで意図的に選んでいる既存パターン（Firebase Admin SDKの標準
Storage機能を使わず、専用バケット・専用サービスアカウントで完結させる）
とは逆方向の提案だった**。

**再提案: 録音Egressと同じ「専用GCSバケット＋専用サービスアカウント」方式
とする（Firebase Storage機能は使わない）**

理由:

1. **既存コードベースとの一貫性**: 録音Egressが既にこの方式で稼働して
   おり、`@google-cloud/storage`・署名付きURL発行・認証情報の環境変数
   経由の受け渡しといった実装パターンをそのまま流用できる
   （`recording.js`の`loadGcsCredentials()`・`getSignedUrl()`等の
   コードをほぼそのまま参考にできる）
2. **Firebase Admin SDKのStorage機能を新たに導入する必要がない**:
   このプロジェクトはこれまで一度もFirebase Admin SDKの`storage()`を
   使っておらず、今回新たに導入すると「録音は`@google-cloud/storage`
   直接、添付ファイルはFirebase Admin SDK経由」という2つの異なる
   GCSアクセス方式が同居することになり、Phase12で問題視した
   「権限チェック・実装方式の分散」を新たな箇所（ストレージアクセス層）
   で再発させてしまう
3. 二十四訂で挙げた「Firebaseコンソールでの一元管理」というメリットは、
   素のGCSバケットであってもGCPコンソール側で同じプロジェクトの他の
   バケットと並べて確認できるため、決定的な差にはならない

**このバケットも録音Egressとは別に新設する（この点は二十三訂の判断を
維持）**。録音とチャット添付では想定読者（owner/moderator限定 vs
Room member全員）・ライフサイクル要件・オブジェクトの性質が異なるため、
「専用サービスアカウント方式を踏襲する」ことと「バケット自体は分離する」
ことは両立する。

**新設バケットの構成案（たたき台。実装着手前に要最終確認）**:
- 環境変数: `RECORDING_GCS_BUCKET`に倣い、`ATTACHMENTS_GCS_BUCKET`
  （バケット名）・`ATTACHMENTS_GCS_CREDENTIALS_JSON`/
  `ATTACHMENTS_GCS_KEY_FILE`（サービスアカウント認証情報）を新設する案
- パス構成: 録音Egressの`recordings/{roomId}/{timestamp}.ogg`という
  実際の命名規則に倣い、`attachments/{roomId}/{messageId}/{filename}`
  とする（二十三訂時点の案`rooms/{roomId}/attachments/...`は実コード
  確認前の推測であり、実際の命名規則により近い形へ修正した）
- 読み取り: クライアント直接のGCS読み取りは許可せず、token-server経由で
  短期署名URLを都度発行する方式とし、BAN即時失効と揃える（BANされた
  瞬間に新規の署名URL発行ができなくなる、という形での間接的な失効。
  発行済みの署名URLそのものを即座に無効化する手段はGCS標準機能のみでは
  限定的なため、有効期限を短く設定することで実質的にカバーする案。
  録音の`download-url`エンドポイントが5分間有効の署名URLを発行している
  のと同様の考え方）
- サイズ上限: 7.3で確定した1件あたり100MBをtoken-server側のアップロード
  受付（署名URL発行）時にバリデーションする
- ライフサイクル: 7.3「保持期間・削除ポリシー」（団体単位・デフォルト
  30日）の確定後、Phase8の録音TTL/ライフサイクル管理と同様の仕組みを
  このバケット向けに個別設定する

**この再提案自体もユーザーの最終確認待ちであり、まだ確定事項ではない**
（二十四訂で一度「Firebase Storage」に確定した判断を覆す提案のため）。

**（2026-07-27、二十七訂で最終確定）** 上記の再提案（専用GCSバケット＋
専用サービスアカウント方式、Firebase Storage機能は使わない）はユーザー
確認済みで確定した。あわせて「録音Egress自体も今回まとめてFirebase
Storageへ移行するか」を検討したが、**今回はEgressには一切手を入れない**
ことで確定した。

検討の過程で「バックアップを見据えた場合、素のGCSとFirebase Storageで
効率差があるか」という質問があったため調査した結果を記録しておく。
両者は実体として同じGoogle Cloud Storageであり、オブジェクトバージョニング・
Storage Transfer Service（バケット間・リージョン間コピー）・デュアル
リージョン/マルチリージョンのバケットロケーション選択といったバックアップ
手段は、素のGCSバケットでもFirebase Storage登録済みバケットでも全く同じ
ように使える。Google CloudのBackup and DR Serviceも調査したが、これは
主にVM・データベース・VMware向けであり、GCSバケット自体を保護対象として
直接バックアップするというより、他ワークロードのバックアップ先として
GCSバケットを使うケースが中心である。したがって、バックアップ効率は
「バケットの種類」ではなく「バケットのロケーション設定・バージョニング等の
設定」に依存し、今回の保存先方式の選択には影響しない。現状の録音バケット
（`ptt-dev-recording-bucket`）・新設する添付ファイル用バケットとも、
バックアップ体制自体は本計画のスコープ外の別途検討事項として残る
（バージョニング等が現在有効になっているかはGCP側の設定次第であり、
リポジトリからは確認できない）。
