...
作成日: 2026-07-09 ／ 最終改定: 2026-08-03（四十六訂）
...
3. **（低優先度・参考記録）Room owner向けバッジ付与/剥奪UIの未実装**：
   Phase13で「APIはあるが3クライアントに操作UIは設けない」と意図的に
   スコープ外とした判断。管理者サイト経由での付与/剥奪運用で当面は
   足りている想定だが、Room ownerが自分の画面から直接操作したいという
   要望が出た場合に再検討する
4. **（2026-08-03 四十六訂で追加）item29のリポジトリへの反映確認**：
   見出し・アイコンの表示を組織名/ルーム名で切り替える変更
   （`RoomView.vue`/`App.vue`・`ContentView.swift`・`PTTApp.kt`）は
   アップロードされたZIPアーカイブに対して実装したのみで、
...
29. ✅ **完了（2026-08-03、四十六訂）**: item27（組織階層のパンくず表示）を
    拡張し、見出し・接続状態アイコンの表示ロジックを「組織(orgId)に
    紐づくRoomは組織名、無所属Roomはルーム名」へ変更した。
    `displayName = orgName ?? roomName`という1つの計算値をWeb・iOS・
    Android3クライアント共通で導入し、`OrgBreadcrumb`系コンポーネントは
    組織名の重複表示をやめてノード階層のみを補助表示する形に整理した。
    リポジトリへの反映確認は次アクションとして残る（item27と同様、
    Xcode/Gradleビルドはこの環境では未実施）
