...
- バッジマスタは、Phase11で導入する組織階層のうち最上位の団体単位で
  ...
  **（2026-07-26 十八訂で発見・要確認）** 「6. 次アクションの提案」item 1
  （十二訂時点の記述）は逆に「団体IDを持たないシンプルな1マスタ構成で
  よい」としており、本項目と矛盾している。`phase13-badge-schema.md`の
  スキーマ案は次アクション側（団体IDなし）の前提で作成したが、どちらを
  正とするかは未確定のため実装着手前に決める必要がある
...
schema.md`)を作成し完了したため「6.1」item 13へ移動した。

1. **【要確認・十八訂で発見】バッジマスタの団体スコープに関する
   ドキュメント内矛盾を解消する**：「3. 優先順位付きロードマップ案」
   Phase13本文は「バッジマスタは団体単位で保持する」としているが、
   旧次アクション(十二訂)は「団体IDを持たないシンプルな1マスタ構成で
   よい」としており、本ドキュメント内で矛盾している。作成済みの
   `phase13-badge-schema.md`は後者（団体IDなし）の前提で設計している
   ため、Phase13実装に入る前にどちらを正とするか確定し、必要であれば
   スキーマ案（`badges`への`orgId`の要否）を修正する
...
3. **admin-dashboardの事前権限チェックの要否を検討する**：Phase12棚卸しで
...
4. **Phase13 バッジ基本機能のFirestoreスキーマの実装着手**：上記item1の
   矛盾解消後、`phase13-badge-schema.md`のスキーマ案（`badges`/
   `badgeGrants`/`config/badgeDisplay`）を土台に、Phase10で実装済みの
   Guestバッジ表示（自分自身のみ）を本バッジシステムの仮想バッジ案へ
   統合するかどうかも合わせて判断した上で実装に入る

### 6.1 完了済みアクション（アーカイブ）

<details>
<summary>2026-07-25〜2026-07-26に完了した旧提案1〜13（クリックで展開）</summary>
...
13. ✅ **完了（2026-07-26、十八訂）**: 「6. 次アクションの提案」item 1
    ...
    - **（重要な留保・要フォロー）** 本スキーマ案は次アクション記載の
      「Phase13時点では団体IDを持たないシンプルな1マスタ構成でよい」を
      前提に設計したが、「3. 優先順位付きロードマップ案」Phase13本文には
      「バッジマスタは、Phase11で導入する組織階層のうち最上位の団体単位で
      保持する」という**逆の前提**が残っており、本ドキュメント内で矛盾して
      いることに気づいた。次アクション側の記述（次アクション作成時点の
      十二訂）の方が新しいが、ロードマップ本文側は未修正のまま残っていた
      可能性がある。どちらを正とするかは実装着手前に確定する必要があり、
      次アクションとして残す（詳細は「3. 優先順位付きロードマップ案」
      Phase13欄の注記、および下記次アクション参照）
