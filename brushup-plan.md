...
## 6. 次アクションの提案（2026-08-02 三十六訂で更新）
...
5. **組織ロースター層の実装着手** 🚧 **バックエンドは検証済み・
   ...
   **2026-08-02、三十四訂で`dev-tools/test-roster.sh`によるHTTPレベルの
   ...
   - **admin-dashboardのブラウザでの実機動作確認（続き）**：三十五訂・
     三十六訂の修正のビルド確認・目視確認・リポジトリ反映に加え、名簿の
     付与/編集/剥奪の実際の画面操作、`firestore.rules`/インデックスの
     本番デプロイ確認が未確認のまま残る
   - 完了後、item4（招待コードの可視範囲）に着手する
6. **（2026-08-02、三十六訂で新規追加・優先度低）** `isForbidden`系フラグの
   機械的な棚卸し：三十五訂・三十六訂とも「画面を1つずつ目視で確認する」
   形で個別のちらつき不具合を発見・修正してきており、同種の入れ子
   セクション（画面トップレベルではなく、画面内の一部セクションだけが
   別ストアの`isForbidden`で出し分けられている箇所）が他にも残っている
   可能性を否定できない。`grep -n "isForbidden" admin-dashboard/src`等で
   全箇所を洗い出し、いずれも「`isLoading && list.length === 0`を最初の
   分岐に置く」パターンに揃っているかを一度機械的に確認しておく
