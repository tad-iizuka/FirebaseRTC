...
6. ✅ **完了（2026-08-11、実機スクリーンショットにより確認）**: チャットUI
   ...
   両方の証跡になる
7. **チャットUI刷新(Android版)の実機ビルド確認・実機IME確認**：ユーザーから
   提示された実機スクリーンショットにより、(a)実機での`assembleDebug`
   再ビルド成功（アバター表示・吹き出し描画を含め正常に起動）は確認
   できた。一方、(b)実機の日本語IME(Gboard等)でのチャット入力時に誤送信が
   起きないかは、今回のスクリーンショットからは確認できない（入力中の
   IME変換候補が見える状態のスクリーンショットが必要）。(a)は完了、(b)の
   IME確認のみ次アクションとして残す
