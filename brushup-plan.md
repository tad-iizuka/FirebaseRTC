...
14. **Web版レイアウト刷新の実装** → **実装完了（2026-08-08、コミット
    `e48725c`・`51ba715`）。「6.1」item35へ移動**。詳細は文書冒頭
    「六十三訂」参照
15. **iOS/Androidのタブレット幅レイアウト** → **iOS/Androidとも実装完了
    ...
    **残課題（六十三訂で(b)(c)を更新・縮小）**：(a) 両OSともXcode/Gradle
    による実際のビルド実行・タブレット実機/エミュレータでのサイズクラス
    切り替え確認が引き続き未実施（Android版の`import
    androidx.compose.foundation.layout.weight`誤りは、修正後のファイルに
    該当importが存在しないことをソース上で直接確認済み＝六十二訂の修正は
    正しく反映されているが、Gradleでの実際のコンパイル実行そのものは
    この環境にSDKが無いため未確認のまま）、
    ~~(b) リポジトリへの反映確認~~ **解消（六十三訂）**：今回取得した
    HEAD（`22e4398`）自体が六十一訂・六十二訂の内容を含む実コミットで
    あることを確認した、
    (c) 未入室中（ルーム選択画面）のタブレット幅専用レイアウトは両OSとも
    スコープ外のまま
16. **PWA化** → **実装完了（2026-08-09、六十五訂）。「6.1」item36へ移動**。
    詳細は文書冒頭「六十五訂」・「3. 優先順位付きロードマップ案」Phase16参照
17. **（更新）iOS版: Liquid Glass関連実装の標準コンポーネントへの回帰・
    ...
    戻した。**（2026-08-09 六十六訂で追加）** 続けて標準TabBar自体の背景
    レンダリング問題への追加対応として、TabViewへ
    `.preferredColorScheme(.dark)`を追加したことをユーザーから報告を受け、
    コミット`bbe774a`で`git show`により反映を確認した。この環境では
    Xcode/実機/シミュレータが無いため、(a)実機での再描画確認（QRボタン等で
    報告されていたフラッシュ現象、および今回の`.preferredColorScheme(.dark)`
    追加後の見た目が解消したか）、(b)解消していない場合の原因切り分け、
    (c)見た目の作り込み（角丸・影等）の再検討、がいずれも未実施のまま
    残っている。詳細は文書冒頭「六十四訂」「六十六訂」参照
