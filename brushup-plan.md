...
2. **（`phase13-badge-schema.md`からの変更点）** 同スキーマ案「8.」は
   ...
   badges/badgeGrants/configいずれもクライアント直接読み取り不可(Admin SDK
   経由のAPIのみ)に統一した。参加者一覧の「最優先1個のみ表示」判定
   (badgeGrantsとbadgesの突き合わせ・Guest仮想バッジの合成)を
...
| **Room owner委譲（2026-08-04、二十一訂で追加）** | バッジ単位の`grantableByRoomOwner`フラグ(デフォルトfalse)。trueのバッジのみ、Room内ownerが自室完結でRoom内owner専用API(`routes/roomBadges.js`)から付与/剥奪できる。falseのバッジは従来通りサイト管理者専用(admin-dashboard、`badges:manage`権限)のまま。role単位の段階分け(moderatorへの一部委譲等)は導入しない単純なON/OFF方式（ユーザー確認済み） |
...
- **admin-dashboard経由のバッジ付与/剥奪がRoomDetailView.vueから
  UsersView.vue(ユーザー管理画面)へ移設されていた（2026-08-04、二十一訂で
  発見。未反映のドキュメント差分）**：本改定作業でアップロードされた
  リポジトリ一式を確認したところ、二十訂時点の本ドキュメントの記述
  （admin-dashboard経由のgrant/revokeは`routes/badges.js`・
  `RoomDetailView.vue`側）とは異なり、`token-server/routes/users.js`・
  `admin-dashboard`の`stores/userDirectory.ts`（2026-07-27新設、コード内
  コメントに経緯記載）へ実際には一本化されていた。「badgeGrantsはRoomに
  紐付かないユーザー単位の概念であり、Room詳細画面から操作するのは
  実装上の都合にすぎず不自然」というユーザー指摘によるものとコード
  コメントに記録されている。今回のRoom owner委譲フラグの実装は、この
  現状（Room内owner専用パスは`routes/roomBadges.js`のまま変更なし、
  admin-dashboard側のみ`users.js`に移設済み）を前提に行った。あわせて、
  `admin-dashboard/src/views/BadgesView.vue`にも2026-08-02日付のコメント
  （`isForbidden`判定のタイミング調整）が残っており、二十訂以降に
  少なくとも2回、本ドキュメントに記載のないコード変更が加えられていた
  ことが分かった。iOS README（六訂）・token-server/README.md（七訂）で
  発生したのと同種の「実コード変更がドキュメントに追随していない」事例の
  再発であり、六訂・七訂ほど実害のある誤判断には至っていないものの、
  記録の完全性という観点では次アクションとして棚卸しの要否を検討する
  余地がある（詳細は「6. 次アクションの提案」item5参照）
...
<details>
<summary>2026-07-25〜2026-08-04に完了した旧提案1〜16（クリックで展開）</summary>
...
16. ✅ **完了（2026-08-04、二十一訂）**: 「Room ownerが自室完結でバッジを
    付与/剥奪したい（例: 当日のリーダーアサイン）」という要望を受け、
    単純にRoom内owner向けUIを実装するだけでなく、「Room内ownerが操作
    できるバッジをバッジ単位で限定する」仕組みをあわせて実装した。
    - **方針確認**: role単位の段階的な委譲（例: moderatorにも一部許可）は
      導入せず、バッジ単位の単純なON/OFFフラグとする方針をユーザーに
      確認した（1択で確定）
    - **`token-server`**: `lib/badges.js`にバッジマスタのフィールドとして
      `grantableByRoomOwner`(boolean, デフォルトfalse)を追加
      (validateBadgeInput/createBadge/updateBadge/badgeDocToJson)。
      `grantBadge`/`revokeBadge`に`viaRoomOwner`引数を追加し、trueの場合は
      対象バッジの`grantableByRoomOwner`がtrueであることを要求する
      (falseの場合は403「Room内ownerによる付与/剥奪を許可されていません」)。
      `routes/roomBadges.js`(Room内owner専用パス)からは`viaRoomOwner: true`
      で呼び出す一方、`routes/users.js`(admin-dashboard経由、サイト管理者
      権限`badges:manage`)は従来通りfalseのままとし、この制約を受けない
      (サイト管理者は委譲フラグに関わらず全バッジを操作できる)。あわせて
      `listRoomOwnerGrantableBadges()`を新設し、`GET /:roomId/badges`が
      呼び出し元がownerの場合のみ`grantableBadges`(付与可能なバッジの
      最小限フィールドの一覧)を返すようにした(badges:monitor権限を
      持たない一般のRoomメンバーへ管理者向け情報を漏らさないため)
    - **`admin-dashboard`**: `types/admin.ts`の`AdminBadge`に
      `grantableByRoomOwner`を追加。`BadgesView.vue`のバッジ新規作成
      フォームにチェックボックスを、既存バッジ一覧に行内トグルボタンを
      追加し、バッジマスタ側でRoom ownerへの委譲可否を設定できるようにした
    - **`ptt-client`(Web)**: `types/api.ts`に`GrantableBadge`型・
      `RoomBadgesResponse.grantableBadges`を追加。`stores/badges.ts`に
      `grantBadgeTo`/`revokeBadgeFrom`アクションを追加(POST/DELETE
      `/rooms/:roomId/members/:targetUid/badges*`を呼び、成功後は
      `fetchOnce`で即時再取得)。`ParticipantList.vue`に、
      `grantableBadges`がnullでない場合(=自分がowner)のみ表示される
      付与/剥奪UI(現在の付与済みバッジの剥奪ボタン、選択式の新規付与)を
      追加し、`RoomView.vue`に配線した。i18nキー(`participants.badge*`)を
      ja/en両方に追加した
    - **未実装のまま次アクションへ**: iOS/Android版のバッジ表示UI
      (次アクションitem3、従来から未実装)、およびiOS/Android版のRoom内
      owner向け付与/剥奪UI(次アクションitem4。token-server側APIは
      グローバルで両OS分も実装済みのため、クライアント側の実装のみが残る)
    - **副次的な発見（未反映のドキュメント差分）**: 実装過程で、
      admin-dashboard経由のバッジ付与/剥奪が二十訂時点の記述
      （`routes/badges.js`・`RoomDetailView.vue`側）から
      `routes/users.js`・`UsersView.vue`(ユーザー管理画面)へ2026-07-27付で
      移設されていたことが、コード内コメントから判明した。本ドキュメントは
      この移設を把握しておらず、二十訂時点の記述のまま更新されていなかった
      （六訂のiOS READMEと同種の、実コード変更のドキュメント未反映）。
      今回の実装はこの現状（Room内owner専用パスは変更なし、admin側のみ
