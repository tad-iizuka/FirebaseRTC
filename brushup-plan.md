...
## 6. 次アクションの提案（2026-08-02 三十七訂で更新）
...
3. **サインインゲート自体を閉じるか（候補(v)(vi)）の要否判断**：
   二十九訂・三十訂で検討した通り、候補(i)〜(iv)（三十二訂で実装完了）は
   「サインイン後に何が見える/できるか」を狭める対応であり、「無関係な
   Googleアカウント保持者がそもそもサインインできてしまうこと」自体は
   未解決のまま残っている。対応する場合の候補は (v) Google Workspace
   ドメイン制限（実装は小さいが、管理者全員が単一ドメインであることが
   前提）、(vi) Firebase Authのblocking function（任意のドメインに対応
   できるが、本プロジェクト初のCloud Functions導入になる。許可リストの
   ブートストラップ問題は三十訂で解消策を確認済み）。実害（各APIは
   `requireAdmin`で別途保護されており、サインインの成立自体はデータへの
   到達可能性を意味しない）と対応コストを踏まえ、着手するかどうかを判断する
4. **招待コードの可視範囲（論点6）→ 組織ロースター層（item5）が前提**：
   `rooms:monitor`権限保有者に「Roomへの参加権を事実上配布できる」権限まで
   広げることになるため、対象権限の絞り込み・監査ログ記録の要否と合わせて
   検討する必要がある（5.4参照）。この論点は組織ロースター層（下記item5）の
   `scopeNodeIds`が実装されて初めて「対象Branch配下のstaffに限定した閲覧
   権限」という選択肢が設計可能になるため、item5の実装着手・完了を前提
   条件とする。item5とは別項目として残すが、実質的な着手順序としては
   item5が先行する
5. **組織ロースター層の実装着手** 🚧 **バックエンドは検証済み・
   admin-dashboardのブラウザ確認は着手・UI課題1件を修正済み
   （2026-08-02、三十五訂）**：
   ...
   **2026-08-02、三十四訂で`dev-tools/test-roster.sh`によるHTTPレベルの
   ...
   三十五訂では、残っていた「admin-dashboardのブラウザでの実機動作確認」に
   着手し、以下を確認・対応した：
   - 権限ゲート（`GET /admin/me`の`permissions`/`managedOrgIds`、
     `App.vue`のトップレベルゲート）が、無関係アカウント・scope限定admin
     いずれについても設計通り動作することをブラウザ操作
     （DevTools Networkタブでの`GET /admin/me`レスポンス確認込み）で確認した
   - 副次的に発見したUI課題（「組織」タブ以外の5画面で、APIが403の場合でも
     検索/新規作成フォームやフィルタ入力欄が表示されたまま残る）を修正した
     （`AdminsView.vue`・`AuditLogsView.vue`・`BadgesView.vue`・
     `RoomsListView.vue`・`UsersView.vue`。詳細は三十五訂本文参照）。
     **（留保）** この修正はビルド・ブラウザでの目視確認・リポジトリへの
     反映のいずれも未実施
   残っているのは以下。
   - ~~三十六訂の修正のリポジトリ反映~~ → **解消済み（2026-08-02、三十七訂）**：
     `git show a8b05c6`で`RoomDetailView.vue`組織セクションの修正が実際に
     コミットされていることを直接確認した（六訂・八訂・十六訂・十九訂と
     同じ確認プロセス）
   - **admin-dashboardのブラウザでの実機動作確認（続き）**：三十五訂・
     三十六訂の修正の`npm run build`（`vue-tsc`）・ブラウザでの目視確認
     （この環境ではnode_modules不在のため実行できず、ユーザー側での確認が
     必要）に加え、名簿の付与/編集/剥奪の実際の画面操作、
     `firestore.rules`/インデックスの本番デプロイ確認が未確認のまま残る
   - 完了後、item4（招待コードの可視範囲）に着手する
6. **（2026-08-02、三十七訂で調査完了・要実装）** `v-else`直下のフォーム類への
   `isLoading`ガード追加：三十六訂で追加した`isForbidden`系フラグの機械的な
   棚卸し（`grep -rn "isForbidden" admin-dashboard/src`、ストア6件・画面7件）
   に着手した結果、三十五訂・三十六訂で「修正済み」とした6箇所
   （RoomsListView・BadgesView・UsersView・AuditLogsView・AdminsView・
   RoomDetailView組織セクション）は、いずれも「`isForbidden`確定前は
   コンテンツ本体を出さない」という大枠の修正自体は正しく効いている一方、
   `v-else`ブロックの先頭に置かれたページ見出しや入力フォーム
   （RoomsListViewの「ルームを新規作成」欄、BadgesViewの「表示設定」欄、
   AuditLogsView/UsersViewの絞込・検索欄など）自体は`isLoading`系フラグで
   ガードされておらず、`isForbidden`の初期値`false`のままフェッチが完了する
   までの間、権限のないユーザーにもこれらのフォームが一瞬描画されてしまう
   ことが判明した（三十五訂・三十六訂が対象としたのは「セクション全体が
   `isForbidden`単独で出し分けられる」パターンで、「`v-else`直下の一部要素
   だけが`isLoading`未ガードのまま残る」より細かい粒度の同種問題は対象外
   だった）。なお`UserDetailView.vue`の`badgesStore.isForbidden`は編集不可の
   警告文のみでフォームの出し分けに関与しないため対象外と判断した。
   対象5画面＋RoomDetailView（計6箇所）それぞれに、既存の「読み込み中...」
   表示と同じ`isLoading && length === 0`相当のガードをページ見出し・
   フォーム側にも追加する（詳細は文書冒頭の三十七訂参照）

### 6.1 完了済みアクション（アーカイブ）

<details>
<summary>2026-07-31までに完了した旧提案1〜17（クリックで展開）</summary>
