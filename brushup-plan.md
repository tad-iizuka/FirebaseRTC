...
## 6. 次アクションの提案（2026-08-02 三十五訂で更新）
   ...
   **2026-08-02、三十四訂で`dev-tools/test-roster.sh`によるHTTPレベルの
   ...
   - 副次的に発見したUI課題（「組織」タブ以外の5画面で、APIが403の場合でも
     ...
     **（留保）** この修正はビルド・ブラウザでの目視確認・リポジトリへの
     反映のいずれも未実施
   残っているのは以下。
   - **admin-dashboardのブラウザでの実機動作確認（続き）**：三十五訂の修正の
     ビルド確認・目視確認・リポジトリ反映に加え、名簿の付与/編集/剥奪の
     実際の画面操作、`firestore.rules`/インデックスの本番デプロイ確認が
     未確認のまま残る
