...
item4（3クライアント共通のRoom内owner向け付与/剥奪UI）のみとなる。
...
参加者一覧に表示されるようになったため）
二十二訂: 2026-07-27（チャット機能に画像・PDF添付を追加したいとの要望を受け、
実装着手前の要件たたき台を新設した「7. Image/File Event（チャット添付ファイル）
要件たたき台」にまとめた。**（重要な留保）** Guestロール・バッジシステム等の
過去の改定では、要件確定後に実コード（`token-server`・3クライアント）を
直接確認したうえで実装・検証を進めてきたが、今回はリポジトリ本体・
ソースコードを一切参照していない。既存のText Event実装
（`token-server/routes/messages.js`・`ChatPanel.vue`/`stores/chat.ts`・
`PTTChatStore.swift`・`PTTChatStore.kt`）の設計方針は「1. 実コード確認済みの
現状」「2-F」の過去の記述を前提として引用したにとどまり、今回新たに
コードを確認したものではない。着手前にまず実コードを直接確認し、前提の
妥当性を検証することを次アクションに含める。ロードマップには「Phase 16」
として仮追加したが、優先順位（Phase9〜15の既存順序に対しどこに割り込むか）
は7.3の未確定要件と合わせてユーザーの意思決定待ちとする）
...
| **バッジシステム(表示・Owner手動付与/剥奪)** | ✅表示/✅付与 | ✅表示/❌付与 | ✅表示/❌付与 | Phase13。表示は2026-07-26(Web、二十訂)・2026-07-27(iOS/Android、二十一訂)で3クライアントとも完了。付与/剥奪のRoom内完結UIは3クライアントとも未実装で、admin-dashboard経由のみ操作可能(次アクションitem3参照) |
...
**（2026-07-27追記、二十二訂）** 上記のうちImage/Fileの2Eventについて、
チャットへの画像・PDF添付を先行実装したいとの要望を受け、要件たたき台を
「7. Image/File Event（チャット添付ファイル）要件たたき台」として新設した。
実装着手前の要件確定・実コード確認が未了のため、ロードマップ上は
「Phase 16」として優先順位未確定のまま仮追加している。
...
### Phase 16: Communication ModelへのImage/File Event追加（チャット添付ファイル）→ **要件検討中・新設（2026-07-27）**
README.mdのCommunication Modelが定義する残りのEvent種別のうち、チャットへの
画像・PDF添付を先行実装したいとの要望を受けて追加した。詳細要件は
「7. Image/File Event 要件たたき台」参照。

**現時点の位置づけ（未確定）**:
- 優先順位・着手条件は未確定。Phase9(安定性)〜Phase15(業界ラベリング)の
  既存順序に対しどこに割り込むかは、7.3の未確定要件（対応ファイル種別・
  サイズ上限・保存先・保持期間・プレビュー方式・コンテンツスキャン要否・
  Guestの送信可否・3クライアントの対応順序）をユーザーと合意してから確定する
- 既存のText Event実装（`token-server/routes/messages.js`＋
  `ChatPanel.vue`/`PTTChatStore.swift`/`PTTChatStore.kt`）の設計方針を
  踏襲できるかの検証も含め、実コードを直接確認する作業自体がまだ未着手
  （次アクション参照）
...
4. **（2026-07-27 二十二訂で追加）Image/File Event（画像・PDF添付）の
   要件確定と実コード確認**：Phase16として仮追加したチャット添付機能に
   ついて、実装着手前に以下を行う。
   - 「7.3 未確定の要件」に挙げた各項目（対応ファイル種別・サイズ上限・
     保存先とTTL・保持期間/削除ポリシー・プレビュー方式・コンテンツ
     スキャン要否・Guestロールの送信可否・3クライアントの対応順序）を
     ユーザーと合意する（Guestロール・バッジシステムの過去の要件確定
     プロセスと同じ進め方）
   - 要件確定後、着手前に`token-server/routes/messages.js`および
     Web/iOS/AndroidのText Event実装（`ChatPanel.vue`/`stores/chat.ts`・
     `PTTChatStore.swift`・`PTTChatStore.kt`）を実コードで直接確認し、
     本ドキュメントが前提としている設計（token-server経由の書き込み
     一本化・Firestoreリアルタイム配信・BAN連動）が実際にその通りかを
     検証してから実装に入る（今回の要件たたき台自体は実コード未確認の
     ため、Phase1完成度に関わる過去の反省——iOS README陳腐化等——と
     同じ轍を踏まないため）

### 6.1 完了済みアクション（アーカイブ）

<details>
<summary>2026-07-25〜2026-07-27に完了した旧提案1〜16（クリックで展開）</summary>
...
## 7. Image/File Event（チャット添付ファイル）要件たたき台（2026-07-27 検討分）

チャット機能（現状はテキストのみ対応）に画像・PDFの送信を追加したいとの
要望を受けて作成した要件たたき台。Guestロール（5節）・バッジシステム
（5.3節）と同様、実装着手前にここで要件レベルの合意形成を行う想定だが、
**本節は現時点でユーザーとの合意前の初期ドラフトであり、実コードの直接
確認も行っていない**。Phase10（Guestロール）・Phase13（バッジ）の要件
確定時とは異なり、今回はリポジトリを参照せず「1. 実コード確認済みの現状」
「2-F」に記載済みの過去の調査結果のみを前提にしている点に留意する。

### 7.1 対象とするEvent種別

README.mdのCommunication Modelでは、画像は`Image`、PDF等の汎用ファイルは
`File`という別々のEvent種別として定義されている。今回の要望（「画像や
PDFを送れるようにしたい」）は実質的にこの2種別を指すため、本節では
`Image`・`File`の2Eventをまとめて検討対象とする（`Location`/`Reaction`/
`System`/`AI Message`は引き続きスコープ外のまま）。

### 7.2 Text Eventの既存設計から継承するかどうかの検討（たたき台）

| 項目 | Text Eventでの設計（1節・F節の記述に基づく） | Image/Fileでの想定（要検討） |
|---|---|---|
| 書き込み経路 | クライアント→token-server（`POST /rooms/:roomId/messages`）に一本化。LiveKitのData Channelは使わない | 同じ経路を踏襲する案。ただしバイナリを扱うため、アップロード自体の経路（token-server中継か、署名URLによるクライアント直PUTか）は別途検討が必要（7.3） |
| 配信経路 | Firestore `rooms/{roomId}/messages` のリアルタイムリスナー | 同上を踏襲する案。メッセージドキュメントに添付ファイルのメタデータ（URL・種別・サイズ等）を持たせる形を想定 |
| モデレーション連動 | `status:'banned'`になった瞬間、`firestore.rules`側で読み取り権限も失う | 同じ強制パターンを踏襲したい。ただしファイル実体を外部ストレージ（GCS等）に置く場合、Firestoreルールだけでは実体側のアクセス制御にならない点に注意（7.4） |
| BAN後の既存投稿の扱い | 現状はテキストも読めなくなるのみで、投稿自体の削除は行わない | 画像・PDFはテキストより「残り続けることの重さ」が異なりうるため、削除要否は別途確認が必要 |

### 7.3 未確定の要件（実装着手前にユーザー確認が必要な項目）

- **対応ファイル種別**: 画像（jpg/png/heic等）・PDFの2種類に限定するか、
  README.mdの`File` Eventが本来想定する任意の拡張子まで許容する汎用File
  添付にするか
- **ファイルサイズ上限**: 1件あたりの上限、Room単位・ユーザー単位の総量
  上限の要否
- **保存先**: 既存の録音Egressと同じGCSバケットを流用するか、専用バケット
  を新設するか。Phase8で実装済みのGCS/FirestoreのTTL/ライフサイクル管理に
  そのまま乗せられるかも要確認
- **保持期間・削除ポリシー**: 録音と同様のTTLに揃えるか、Room解散と同時に
  削除するか
- **プレビュー方式**: チャット欄に画像サムネイル/インライン表示をするか、
  ダウンロードリンクのみとするか（特にPDFはプレビュー実装コストが高い）
- **コンテンツスキャン**: 悪意あるファイル（マルウェア等）や不適切な画像の
  スキャンを行うか。行う場合、アップロード経路の設計（token-server中継か
  署名URL方式か）に直結する
- **Guestロールの送信可否**: Guestは送話可能・テキストチャットも実質可能
  という現行設計だが、画像・PDF送信も同様に許可するか、Guestは対象外と
  するか
- **3クライアントの対応順序**: Text Event実装時と同様に3クライアント同時
  実装とするか、Web先行など段階的にするか

### 7.4 洗い出された懸念点

- Firestoreドキュメントに直接バイナリを埋め込む設計は現実的でなく、GCS等の
  外部ストレージ＋メタデータ参照という構成が妥当と考えられる。その場合、
  BAN即時失効と同水準の強制力をファイル実体側にも及ぼす手段（署名URLの
  短期失効・アクセス制御の設計）を別途検討する必要がある
- Phase1（警備業）を主対象とするなら、現場写真等の送信ニーズは実利用価値が
  高い一方、Phase9で優先している「安定性・バックグラウンド動作」等の
  残課題との優先順位付けをユーザーと合意する必要がある（Phase16の
  ロードマップ上の位置づけが未確定なのはこのため）
- 本節の内容は実コード未確認のドラフトであるため、次アクション（「6.」
  item 4）で実コード確認を行った結果、Text Eventの実装が本節の想定と
  異なっていた場合は本節を訂正する
