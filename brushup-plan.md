...
## 6. 次アクションの提案（2026-08-02 三十八訂で更新）
...
6. **（2026-08-02、三十八訂で実装・要検証）** item 6「`v-else`直下のフォーム類
   への`isLoading`ガード追加」を実装した。`RoomsListView.vue`（「ルームを
   新規作成」欄）・`BadgesView.vue`（「表示設定」欄・「バッジを新規作成」欄）・
   `AuditLogsView.vue`（絞込フォーム）・`AdminsView.vue`（付与/剥奪フォーム）
   の4画面5箇所に、既存の「読み込み中...」表示と同じ`isLoading &&
   length === 0`相当のガードを追加した。`RoomDetailView.vue`組織セクションは
   三十六訂で既に対応済みのため対象外。`UsersView.vue`は自動フェッチが無く
   実害は無いと判明したが、他画面とのパターン統一のため予防的に同じガードを
   追加した（詳細は文書冒頭の三十八訂参照）。
   残作業: この環境では`npm run build`（`vue-tsc`）・ブラウザでの目視確認が
   実行できないため、ユーザー側での型チェック・目視確認・リポジトリへの
   反映確認が必要
