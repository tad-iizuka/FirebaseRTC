...
- Guest→Member昇格導線 → **対象外（2026-07-25 確定）**
  - 昇格導線・APIは実装しない
  - GuestIDは生成された状態のまま保持し、他ID体系との紐付け・統合は
    一切行わない
  - 監査ログ上もGuestとMemberは常に別ID・別人物の記録として扱う
...
5. **Guestロールの実装着手（2026-07-25 方針確定・着手可）**：
   「5. Guestロール・バッジシステム 詳細仕様」で要件は確定済み。
   5.4で保留していたGuestIDとMember昇格時のID整合性についても、
   **Member昇格は対象外（実装しない）、GuestIDは生成されたIDのまま
   保持し他ID体系との紐付けも行わない**ことで確定した。この前提で
   Phase10の実装（`token-server`のrole関連API・Firestoreルール・
   3クライアントUIへのGuest導線）に着手する
