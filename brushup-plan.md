...
## 6. 次アクションの提案（2026-07-31 三十一訂で更新）
...
して「6.1」item 16へ移動した（詳細は文書冒頭の二十五訂参照）。二十六訂では、
item3（論点5・6）について論点5の具体的な現象を確認し内容を刷新した
（詳細は文書冒頭の二十六訂参照）。あわせて、論点5と論点6・item4（組織
ロースター層）の依存関係を整理した：論点6は組織ロースター層（`scopeNodeIds`）
が入って初めて「`rooms:monitor`より狭い閲覧権限」を設計できるようになる
という前提条件の関係にあり、論点5とは独立している。二十七訂では、
`AuthView.vue`の未ログイン時説明文にも同種の情報露出（必要な権限名・
Firestoreパスに加え、権限付与用の内部スクリプト名まで）があることが
判明し、item3にこの4点目を追加した（詳細は文書冒頭の二十七訂参照）。
二十八訂では、item3の対応候補(iii)（ログイン後の権限チェック追加）を
実施した場合の「最初の管理者を誰が任命するか」という懸念について、
`dev-tools/grant-admin-permission.js`がadmin-dashboardのサインインフロー
を経由しない独立した経路であることを確認し、ブロッカーには当たらないと
結論した（詳細は文書冒頭の二十八訂参照）。二十九訂では、候補(iii)(iv)を
実施しても「無関係なGoogleアカウント保持者がそもそもサインインできて
しまうこと」自体は解消されないことを確認し、これを別論点として切り分けた
（詳細は文書冒頭の二十九訂参照）。三十訂では、候補(vi)（blocking function）
の許可リストのブートストラップ問題が、既存の`dev-tools`スクリプトの
延長で解消できることを確認した（詳細は文書冒頭の三十訂参照）。三十一訂
では、「最初の管理者さえ登録すればあとはadmin-dashboardで対応できる」
という見立てを確認し、`admins:manage`自体の付与/剥奪だけは恒久的に
`dev-tools`が必要であり続けるという例外を明確化した（詳細は文書冒頭の
三十一訂参照）。残る項目は以下の4件。
...
3. **admin-dashboardの事前権限チェック（論点5）の対応方針決定**：
   十八訂で実装したのは棚卸しの論点1・2・4、および新たに見つかった論点8
   （対応表未配線の4操作）のみで、論点5は「実装ではなく仕様判断が先に
   必要」として保留にしていた。**（2026-07-31、二十六訂・二十七訂で
   具体化）** 実コード確認の結果、以下4点を確認した（詳細は文書冒頭の
   二十六訂・二十七訂参照）。
   - 未ログイン時の画面(`AuthView.vue`)にトークンサーバーURL入力欄が
     常時表示されている
   - `signInWithGoogle()`・`App.vue`いずれも権限チェックを行っておらず、
     `adminUsers/{uid}.permissions`の有無に関わらず、任意のGoogleアカウント
     でサインインするだけで全タブ(`NavTabs`)のメニュー構成が閲覧できる
   - 権限を持たない状態で各画面を開くと、`AdminsView.vue`等複数の画面が
     Firestoreのフィールドパス・必要な権限文字列をそのままエラーメッセージ
     に表示しており、内部の権限体系が外部から探索可能になっている
   - **（二十七訂で追加）** `AuthView.vue`の未ログイン時説明文自体にも、
     必要な権限(`rooms:monitor`)・Firestoreパスに加え、権限付与に使う
     内部スクリプト名(`dev-tools/grant-admin-permission.js`)まで、認証不要
     で誰でも閲覧できる形で記載されている。4点のうち最も露出範囲が広い
   
   各APIエンドポイント自体は`requireAdmin`ミドルウェアで別途保護されており
   実データへのアクセスは防がれているが、インフラ情報(URL)・内部の権限体系
   （権限名一覧・Firestoreスキーマ・権限付与手順）が露出している点は、
   対応するかどうかの判断が必要。対応する場合の候補は (i) 未ログイン時の
   URL非表示、(ii) `AuthView.vue`の説明文の削除、または権限名・スクリプト名
   を伏せた一般的な文言への置き換え、(iii) ログイン後・各画面表示前の権限
   チェック追加、(iv) エラーメッセージから権限名・Firestoreパスを除去、
   の4点。**論点6（招待コードの可視範囲）とは独立した論点であり、論点5単独
   で判断・着手できる**（論点6はitem4の組織ロースター層の実装を前提とする
   ため、着手時期がitem4に依存する）。**（2026-07-31、二十八訂で確認）**
   候補(iii)を実施すると最初のサイト管理者を誰も任命できなくなるのでは
   という懸念があったが、`dev-tools/grant-admin-permission.js`が
   admin-dashboardのGoogleサインインフロー・権限チェックを一切経由しない
   独立したローカルCLI（Admin SDK・サービスアカウント認証）であることを
   確認し、(iii)実施の障害にはならないと判断した。運用上は「対象者が一度
   Googleでサインインしuidを確定させる → サービスアカウントへのアクセス権
   を持つ運用者がスクリプトで`rooms:monitor`等を付与する」という2段階の
   手順になる。**（2026-07-31、二十九訂で追加確認）** 一方、候補(i)〜(iv)
   はいずれも「サインイン後に何が見える/できるか」を狭める対応であり、
   「サインインという行為自体を、無関係なGoogleアカウント保持者に対して
   閉じるか」は別論点であることが判明した。現状`GoogleAuthProvider`に
   ドメイン制限は無く、Firebase Authのblocking function（`beforeSignIn`
   等）を実装する`functions/`ディレクトリもリポジトリに存在しないため、
   候補(i)〜(iv)を全て実施しても任意のGoogleアカウントでのサインイン自体は
   引き続き成立する。これを閉じたい場合の追加候補は以下の2つで、いずれも
   (i)〜(iv)より大きい変更を伴う。
   - (v) **Google Workspaceドメイン制限**：`setCustomParameters({ hd: ... })`
     追加のみで実現可能だが、管理者全員が単一Workspaceドメインのアカウント
     であることが前提（個人Gmailの管理者がいる場合は不可）
   - (vi) **Firebase Authのblocking function**：任意のメールドメインに対応
     できるが、本プロジェクトで初めてのCloud Functions導入となる
     （token-serverは通常のNode/Expressサーバーであり、blocking function
     に相当するものは現状無い）。**（2026-07-31、三十訂で解消）** 当初
     懸念していた「許可リストのブートストラップ」問題は、判定を`uid`では
     なく`email`をキーにした小さなコレクション（例: `adminInvites/{email}`）
     で行い、その書き込みを既存の`dev-tools/grant-admin-permission.js`と
     同じ立て付け（Admin SDK・サービスアカウント認証のローカルCLI、既存
     スクリプトへのサブコマンド追加程度）で行うことで解消できると判断した。
     `phase11-org-roster-design.md`「6.2」の名簿招待動線（メールアドレスで
     事前登録し、本人の初回サインアップ時点で紐づく）と同型のパターンであり、
     ロースター層の設計を転用できる。残る論点は「Cloud Functionsという
     新しい運用対象を追加するコストに見合うか」のみに絞り込まれた
   
   (v)(vi)の要否は、(i)〜(iv)の対応方針決定時にあわせて判断する
4. **招待コードの可視範囲（論点6）→ 組織ロースター層（item4）が前提**：
   `rooms:monitor`権限保有者に「Roomへの参加権を事実上配布できる」権限まで
   広げることになるため、対象権限の絞り込み・監査ログ記録の要否と合わせて
   検討する必要がある（5.4参照）。**（2026-07-31時点の整理）** この論点は
   組織ロースター層（下記item5）の`scopeNodeIds`が実装されて初めて
   「対象Branch配下のstaffに限定した閲覧権限」という選択肢が設計可能になる
   ため、item5の実装着手・完了を前提条件とする。item5とは別項目として
   残すが、実質的な着手順序としてはitem5が先行する
5. **組織ロースター層の実装着手**：
...
<details>
<summary>2026-07-31までに完了した旧提案1〜16（クリックで展開）</summary>
