...
| バックグラウンド動作(送受信) | - | ✅実機確認済み（主要シナリオ） | ⚠️実装済み・実機未検証 | 2026-07-27、十四訂で実装。iOSは2026-07-30、二十訂でユーザー実機（スクリーンショット提示）により通話継続・ハンドセットボタン復帰・常駐通知表示を確認。長時間/電力最適化・Bluetoothヘッドセット単体は未確認。Androidは引き続き未検証 |
...
- ~~iOS: バックグラウンド動作の実機検証・本実装~~
  ...
  バックグラウンドを往復しても通話が継続すること、の3点を
  スクリーンショット付きで確認した（「現状の実装です」＝十四訂時点の
  既存コードによる挙動であり、追加実装は行っていない）。
  ⚠️ **未完了（範囲を絞り込み）**: Android実機検証（未報告）、長時間
  バックグラウンド接続維持・Doze/App Standby等の電力最適化の影響、
  Bluetoothヘッドセット単体での送話操作、および`git show`等による
  コミット内容の直接検証はいずれも未実施。次アクションとして残す。
...
- **業界ラベリング層をロードマップ後方へ移動（2026-07-26）**：
  ...
  （優先順位と着手条件を変更しただけ）
- **バックグラウンド動作の実機検証・ビルド確認 → iOSは主要シナリオ確認済み
  （2026-07-27課題化 → 2026-07-30、二十訂で一部解消）**：十四訂で実装した
  iOS/Androidのバックグラウンド動作(送受信両対応)について、iOSは
  ユーザーの実機（スクリーンショット提示）で常駐通知表示・ハンドセット
  ボタンでのフォアグラウンド復帰・バックグラウンド往復後の通話継続を
  確認した。一方、以下は引き続き未確認のため次アクションとして残す。
  - Android実機検証（`PTTForegroundService`、未報告）
  - 長時間バックグラウンド接続維持・Doze/App Standby等の電力最適化
    機能(メーカー独自のものを含む)による強制終了の有無
  - Bluetoothヘッドセット単体での送話操作
  - `git show`等によるコミット内容の直接検証（六訂・八訂・十六訂・
    十九訂と同様の確度での確認）

---

## 6. 次アクションの提案（2026-07-30 二十訂で更新）

旧item 5「十八訂の変更のリポジトリへの反映確認」は、`git show`によるコミット
`d802de0`の直接検証をもって完了したため「6.1」item 13へ移動した。item 1は
二十訂でのiOS実機確認を受けて範囲を絞り込んだ。残る項目は以下の4件
（番号は変更なし）。

1. **バックグラウンド動作(十四訂)の残る確認範囲**：iOSの主要シナリオ
   （常駐通知表示・ハンドセットボタンでのフォアグラウンド復帰・
   バックグラウンド往復後の通話継続）は二十訂でユーザー実機確認済み。
   残るのは (a) Android実機検証（`PTTForegroundService`、未報告）、
   (b) 長時間バックグラウンド接続維持・Doze/App Standby等の電力最適化
   機能の影響、(c) Bluetoothヘッドセット単体での送話操作、(d) `git show`
   等によるコミット内容の直接検証。問題があれば都度この計画へ反映する
...
<details>
<summary>2026-07-30までに完了した旧提案1〜13（クリックで展開）</summary>
