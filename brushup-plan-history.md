# ブラッシュアップ計画 改定履歴・完了アクション アーカイブ

対象リポジトリ: `tad-iizuka/FirebaseRTC`
作成日: 2026-08-11（`brushup-plan.md`の整理に伴い、七十一訂までの全改定履歴と
「6.1 完了済みアクション」を本ファイルへ退避した）

> **このファイルの位置づけ**: `brushup-plan.md`は初回作成（2026-07-09）から
> 七十一訂（2026-08-11）まで、改定のたびに変更理由・検証プロセス・確度
> （`git show`による直接検証 / GitHub Actions実行結果 / ユーザー申告のみ、
> 等）を本文冒頭に逐語で積み増す運用をしてきた。実装がPhase16まで進み
> `brushup-plan.md`自体が4400行を超えて肥大化したため、2026-08-11に整理を
> 行い、以下の方針で本ファイルへ分離した。
>
> - **Phase単位の実装内容そのもの**は `CHANGELOG.md` に、**個々の設計判断
>   （なぜそう決めたか）**は `DECISIONS.md` に、すでに整理済みの形で存在する
>   （2026-08-11、七十一訂で作成）。今後の変更はまずそちらを更新する。
> - 本ファイルは、`CHANGELOG.md`/`DECISIONS.md`には残らない**細かい経緯**
>   （検証の確度の違い、途中で発見された陳腐化コメントの実例、誤判断からの
>   訂正プロセスそのもの等）の**監査証跡**として、七十一訂時点の全文をそのまま
>   保存する。今後このファイルへの追記は行わない（凍結）。
> - 現在の実装状況・現在有効な次アクションは `brushup-plan.md` 本体、および
>   `REQUIREMENTS.md`・`API.md`・`DATA_MODEL.md`・`SECURITY.md`・`UI_UX.md`・
>   `ARCHITECTURE.md`・`ROADMAP.md`・`TESTING.md`を参照。

---

## 第1部: 改定履歴（初訂〜七十一訂、`brushup-plan.md`旧冒頭部分）
...
五十九訂: 2026-08-08（Web版(`ptt-client`)のレイアウト刷新について仕様検討を
実施。実装着手前の合意形成段階であり、コード変更は無い。

**背景**: ユーザーからスクリーンショット（デスクトップブラウザでの表示）の
共有を受け、現行のWeb版レイアウトが「ルーム情報→LEAVE ROOM→PRESS TO
TALK→PARTICIPANTS→CHAT」という単一の縦積み構成のままであり、チャット
利用時にPTTボタンが画面外に出る、デスクトップの横幅を活かせていない、
という課題を確認した。五十三訂では「Web版はデスクトップ幅前提の縦積み
レイアウトのままで問題ない」と判断していたが、今回チャットUI（五十六訂〜
五十八訂）が本格的な機能になったことで、その前提が崩れていることが判明した
ため、五十三訂の当該判断を実質的に更新するものである。

**検討したレイアウト方針**:

- ブレークポイント: 768px未満をMobile、768px以上をDesktop/Tabletとする
  2区分（中間状態は設ける）
- **Mobile幅（〜767px）**: iOS/Android版（五十三訂で確認したtalk/chat/
  people/settingsの4タブ構成）に合わせ、Web版にも「通話」「参加者」
  「チャット」のタブ方式を導入し3プラットフォームで操作体系を統一する。
  ただしPTTは警備現場での安全要件（README.mdのPhase1「安定性・低遅延」
  原則）と直結するため、「参加者」「チャット」タブを開いていても押せる
  常設のPTTミニバー（画面下部固定、高さ56px程度の横長バー）を設ける。
  「通話」タブ内は従来通り大きな円ボタンを主役として表示し、ミニバーとは
  二重表示させない
- **Desktop/Tablet幅（768px以上）**: 左ペイン（参加者一覧・常時表示）／
  中央ペイン（PTTボタン・固定・常時表示）／右ペイン（チャット・常時表示）
  の3ペイン構成とし、スクロール無しで全機能を一画面に収める。左ペイン幅
  固定240px程度、右ペイン可変(min 320px)、中央は残り全部
- **PWA化**: `manifest.json`（`display: standalone`・既存ダークテーマに
  合わせた`theme_color`/`background_color`・アイコン一式）とService
  Workerを導入する。Service WorkerはApp Shell（HTML/CSS/JS本体）のみ
  キャッシュ対象とし、Firestore/LiveKitの通信はキャッシュ対象外とする
  （キャッシュすると古い録音状態・BAN状態を見せる事故に直結するため
  明示的に除外）。Web Push通知は今回のスコープ外とし、Phase14
  （プッシュ通知）着手時にService Worker基盤を再利用する形で合流させる
- コンポーネント（PTTStore/ChatStore等）自体のロジック変更は不要で、
  既存コンポーネントを流用しコンテナのCSS（Flex/Grid）のみをブレーク
  ポイントで切り替える設計とした

**（重要）iOS/Androidのタブレット幅レイアウトについて**: ユーザーより、
iOS/Androidも現状タブレット幅（iPad・Androidタブレット等の広い画面）での
レイアウトは未検討のままであり、将来的にはWeb版で検討中の3ペイン
レイアウト（参加者/PTT/チャット）と同様の構成にするのが望ましい、との
方針が示された。五十三訂で確認したiOS/Androidの4タブ構成（talk/chat/
people/settings）はスマートフォン幅を前提にしたものであり、タブレット幅
での挙動については本ドキュメントでこれまで一度も検討されていなかった
（新規に判明した課題として記録する）。

**今回のスコープ外として明示的に確認した項目**:

- Web版コンポーネント内部のロジック変更（PTTStore/ChatStore等）
- Web版の768px以上における中間的な段階レイアウト（一律3ペインとし、
  専用の中間状態は設けない）
- Web Push通知（Phase14へ）
- 実装そのもの（今回は仕様検討のみ。実装着手は次アクションとして残す）

**ワイヤーフレーム**: デスクトップ幅（3ペイン）・モバイル幅（タブ＋PTT
常設バー）それぞれについて、Claude側の可視化ツールでラフなワイヤーフレーム
を作成し、ユーザーと方向性を確認済み（成果物としてのファイルはなし、
チャット上での確認のみ）。

**次アクションへの影響**: 「6. 次アクションの提案」に、(1) 今回の仕様に
基づくWeb版レイアウト実装、(2) iOS/Androidタブレット幅レイアウトの検討
（Web版3ペインと同様の構成を目指す方針）、(3) PWA化（`manifest.json`・
Service Worker）の3件を新規項目として追加する。
...
六十三訂: 2026-08-08（アップロードされたリポジトリ一式（HEAD=`22e4398`。
六十二訂と同一）の`.git`履歴を、ユーザー依頼により改めて直接検証した。

**発見**: 「6. 次アクションの提案」item14「Web版レイアウト刷新の実装」は、
五十九訂で「実装着手前（コード変更は無い）」と記録し、六十二訂時点まで
一度も完了扱いにされていなかったが、`git log`で確認したところ**実際には
既に実装され、リポジトリへコミット済み**であることが判明した。

- 該当コミット: `e48725c`（2026-08-08 10:39、`App.vue`・`RoomView.vue`・
  `ChatPanel.vue`・新規`PttMiniBar.vue`・`ja.json`/`en.json`への
  `tabCall`/`tabParticipants`/`tabChat`キー追加）、`51ba715`（同10:46、
  `RoomView.vue`のチャットペイン表示条件の微修正1行）
- これらは、五十九訂のドキュメント作成コミット`fe72af0`（同10:24）と、
  iOS版タブレット対応を実装した六十訂のコミット`1ddcc72`（同11:02）の
  間、約15〜38分の間に行われていた。六十訂自身が取得したリポジトリの
  HEAD（`51ba715`）は既にこの実装を含んでいたにもかかわらず、六十訂〜
  六十二訂のいずれの改定でもこの事実が記載されないまま見過ごされていた
- **実装内容が五十九訂の合意仕様と一致することをソースコード直接確認で
  検証した**:
  - ブレークポイント768px（Tailwindの`md:`既定値と一致）
  - Mobile幅（`RoomView.vue`）: 「通話」「参加者」「チャット」の3タブ
    （`t('room.tabCall')`等）＋タブ切替中も送話可能な常設PTTミニバー
    （新規`PttMiniBar.vue`、高さ`h-14`=56px、「通話」タブ表示中は
    `v-if`で非表示にし二重表示を回避）
  - Desktop/Tablet幅（768px以上）: 左ペイン（参加者一覧・`md:w-[240px]`
    固定）／中央ペイン（PTT・`md:flex-1`可変）／右ペイン（チャット・
    `md:w-[380px] md:min-w-[320px]`）の3ペイン同時表示
  - 既存コンポーネント（`PttButton`・`ParticipantList`・`ChatPanel`）は
    そのまま流用し、コンテナ側のFlex/Grid・`hidden`の付け外しのみで
    配置と表示/非表示を切り替える設計（五十九訂で合意した「ロジック側は
    変更不要」という方針の通り、`ChatPanel.vue`の変更もレイアウト用
    クラス（`flex`・`min-h-0`・`shrink-0`等）の追加のみで、
    メッセージ送受信ロジック自体への変更は無いことを確認した）
  - 一方、五十九訂の仕様に含まれていた**PWA化（`manifest.json`・
    Service Worker）は今回の実装対象に含まれておらず**、`ptt-client`
    内に該当ファイルが存在しないことを確認した。こちらはitem16として
    引き続き未着手のまま次アクションに残す（ドキュメントの記述と実装
    状態が一致しているのはitem16のみで、item14側に齟齬があった）
- **ビルド確認**: この環境は引き続きネットワーク遮断のため
  `npm install`が`registry.npmjs.org`への403エラーで失敗し、
  `vue-tsc -b`・`eslint .`の実行はできなかった。ソースコードの目視
  確認のみで完了扱いとする（六訂以降繰り返し記録している、この環境の
  既知の制約と同様）
- 併せて、item15（iOS/Androidタブレット幅レイアウト）の残課題のうち
  (b)「リポジトリへの反映確認」についても、今回取得したHEAD（`22e4398`、
  六十一訂・六十二訂の内容を含む）自体が既にリポジトリの実コミットで
  あるため、`git show`によらずとも確認済みとして解消する。(a)の
  Android版`weight`import誤りについても、修正後のファイルに該当import
  が存在しないことをソース上で直接確認した（六十二訂の修正が正しく
  この状態に反映されている）。ただし**Gradle/Xcodeによる実際のビルド
  実行そのものは、この環境にSDK/ツールチェインが無いため依然として
  未確認**であり、(a)の残課題はここまで縮小した上で残す

**次アクションへの影響**: 「6. 次アクションの提案」item14を「6.1」の
...
六十六訂: 2026-08-09（アップロードされたリポジトリ一式（HEAD=`bbe774a`）を
取得し、`.git`履歴を直接検証した。あわせて、ユーザーから新たに
`ContentView.swift`のTabViewへ`.preferredColorScheme(.dark)`を追加する
修正の報告を受け、反映内容を確認した。

**(1) item18（PWA化のリポジトリ反映確認）の解消**: `git log --oneline`で
`5d0ab05`（六十五訂の実装対象）以降のコミットを遡ったところ、コミット
`ea965f0`（2026-08-09 11:36:08、`ptt-client/index.html`・`public/
apple-touch-icon.png`・`public/favicon.ico`・`public/icons/*`・
`public/manifest.json`・`public/sw.js`・`src/main.ts`の計11ファイル、
191行追加のみ）が、六十五訂で実装した内容とファイル単位で完全に一致する
ことを`git show --stat`で確認した。六訂・八訂等と同じ`git show`による
直接検証であり、これによりitem18は解消したものとして「6.1」item37へ
移動する。

**(2) iOS Liquid Glass回帰（六十四訂）のリポジトリ反映も確認**: 同様に
コミット`c956e72`（2026-08-09 16:56:03、`ptt-ios/ptt-ios/ContentView.swift`・
`brushup-plan.md`の2ファイル）が、六十四訂で記述したタブバー・サインイン
画面カード・ボタン5箇所の標準コンポーネントへの回帰内容と一致することを
確認した。ただしitem17が求めているのは実機での見た目確認（フラッシュ現象が
実際に解消したか）であり、これはリポジトリの情報だけでは判断できないため、
item17自体は完了扱いにせず残す。

**(3) `.preferredColorScheme(.dark)`の追加（ユーザーからの直接報告）**:
コミット`bbe774a`（2026-08-09 17:31:04、`ContentView.swift`1行追加のみ）を
`git show`で確認したところ、六十四訂で標準実装へ戻した`TabView(selection:
$selectedTab)`の`.tint(.pttAccent)`直後に`.preferredColorScheme(.dark)`が
追加されていた。ユーザーからの説明によれば、標準TabBar自体の背景
レンダリング（Liquid Glass関連）の問題に対する追加対応とのことで、アプリ
全体が元々ダークテーマ専用の設計（`.pttBackground`/`.pttAccent`等のデザイン
トークンで統一）であるのに対し、標準`TabView`はOSのカラースキーム設定に
応じてTabBar背景を動的に描画するため、システム側の設定次第でアプリの
デザイントークンと噛み合わない背景になり得る、という状況を
`.preferredColorScheme(.dark)`で固定することで解消する対応と理解した。
`ContentView.swift`内に他の`colorScheme`関連の指定は無く、今回の1行が
唯一の変更であることも確認した。

**（重要な留保）** この対応がQRボタン等で報告されていた実際のフラッシュ
現象を解消したかどうかは、リポジトリの情報だけでは判断できず未確認のまま
である。item17（六十四訂の実機確認）に、この追加対応の確認も含める形で
更新する。

**次アクションへの影響**: item18は「6.1」item37へ移動して解消。item17の
記述に今回の`.preferredColorScheme(.dark)`追加を反映し、実機確認の対象を
六十四訂の標準コンポーネント回帰と合わせて更新する。
...
34. ✅ **完了（2026-08-05、五十四訂）**: Room開始/終了時刻(Schedule)機能を
    ...
    として残る）
35. ✅ **完了（2026-08-08、コミット`e48725c`・`51ba715`。六十三訂で発見・
    確認）**: Web版レイアウト刷新。五十九訂で合意した仕様（768px
    ブレークポイント、Mobile幅は通話/参加者/チャットのタブ方式＋常設
    PTTミニバー`PttMiniBar.vue`(新規、高さ56px)、Desktop/Tablet幅は
    参加者(240px固定)/PTT(可変)/チャット(380px・min320px)の3ペイン構成）
    に基づき`App.vue`・`RoomView.vue`・`ChatPanel.vue`が実装・コミット
    済みであることを`git log`/`git show`で直接検証した。**（留保）**
    この実装作業自体はユーザー側で行われたもので、五十九訂〜六十二訂の
    いずれの改定でも本ドキュメントに記載されないまま完了していた
    （六十訂が取得したHEAD`51ba715`は既にこの実装を含んでいたが、
...
36. ✅ **完了（2026-08-09、六十五訂）**: 旧item16「PWA化」を実装した。
    ...
    として残す
37. ✅ **完了（2026-08-09、六十六訂、`git show`による直接検証済み）**:
    item18「Phase16(PWA化)のリポジトリ反映確認」。アップロードされた
    リポジトリ一式（HEAD=`bbe774a`）を取得し、`git show --stat ea965f0`で
    六十五訂が実装した11ファイル（`ptt-client/index.html`・`public/
    apple-touch-icon.png`・`public/favicon.ico`・`public/icons/*`・
    `public/manifest.json`・`public/sw.js`・`src/main.ts`）が同一コミット
    として実際にリポジトリへ反映済みであることを確認した。あわせて、
